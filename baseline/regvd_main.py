...
def test(args, model, tokenizer, test_dataset, best_threshold):
    # build dataloader
...
def main():
    ...
    # Set seed
    set_seed(args)
    config = RobertaConfig.from_pretrained(args.config_name if args.config_name else args.model_name_or_path, trust_remote_code=True)
    tokenizer = RobertaTokenizer.from_pretrained(args.tokenizer_name, trust_remote_code=True)
    model = RobertaModel.from_pretrained(args.model_name_or_path, trust_remote_code=True) 

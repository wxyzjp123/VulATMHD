...
def main():
    ...
    parser.add_argument("--lambda_triplet", default=0.4, type=float,
                    help="Weight coefficient for the triplet loss.")
    ...
    # Set seed
    set_seed(args)
    tokenizer = RobertaTokenizer.from_pretrained(args.config_name if args.config_name else args.model_name_or_path, trust_remote_code=True)
    tokenizer.add_tokens(["<dis>"])
    tokenizer.dis_token_id = tokenizer.encode("<dis>", add_special_tokens=False)[0]
    tokenizer.dis_token = "<dis>"
    codebert = RobertaModel.from_pretrained(args.model_name_or_path, trust_remote_code=True)

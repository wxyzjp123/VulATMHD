...
from codebert_model import StudentBERT
import pandas as pd
from sklearn.metrics import accuracy_score, f1_score
from textcnn_model import TextCNN
from feature_model import CNNTeacherModel
...
def train(args, train_dataset, model, teacher_model, tokenizer, eval_dataset, cwe_label_map):
    ...
    student_crd_embed_dim = 768
    teacher_crd_embed_dim = 300
    crd_loss_fn = CRDLoss(
            student_feat_dim=student_crd_embed_dim, 
            teacher_feat_dim= teacher_crd_embed_dim, 
            temperature_crd= args.crd_temp,
            use_projection= (student_crd_embed_dim != teacher_crd_embed_dim) 
        ).to(args.device)
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            teacher_logits, teacher_embedding_for_crd = teacher_model(input_ids=input_ids, groups=groups, labels=labels, return_logit=True)
            ...
            if args.use_hard_distil:
                loss, loss_dis, student_embedding_for_crd = model(input_ids=input_ids, 
                                        labels=labels, 
                                        soft_label=None,
            ...
            loss_crd = torch.tensor(0.0, device=args.device) # 初始化
            if crd_loss_fn is not None and teacher_embedding_for_crd is not None:
                current_loss_crd = crd_loss_fn(student_embedding_for_crd, teacher_embedding_for_crd, args.device)
                if args.n_gpu > 1:
                    current_loss_crd = current_loss_crd.mean()
                if args.gradient_accumulation_steps > 1:
                    current_loss_crd = current_loss_crd / args.gradient_accumulation_steps
                loss_crd = current_loss_crd

            total_loss = loss + args.lambda_dis * loss_dis + loss_crd
            total_loss.backward()
...
def test(args, model, tokenizer, test_dataset, beta, group_label_map):
    # build dataloader
...
def main():
    ...
    parser.add_argument("--margin", default=0.3, type=float,
                    help="Margin value for adaptive triplet mining.")
    parser.add_argument("--crd_temp", default=0.1, type=float,
                    help="Temperature parameter.")
    parser.add_argument("--lambda_dis", default=0.5, type=float,
                    help="Weight coefficient for the distillation loss.")
    ...
    with open("data/BigVul/cwe_label_map.pkl", "rb") as f:
    ...
    sub_checkpoint = args.model_name_or_path
    tokenizer = RobertaTokenizer.from_pretrained(sub_checkpoint, trust_remote_code=True)
    ...
    codebert = RobertaModel.from_pretrained(sub_checkpoint, trust_remote_code=True)
    ...
    model = RobertaModel.from_pretrained(sub_checkpoint, trust_remote_code=True)
    ...
    if args.do_test:
        ...
        y_trues, y_preds = test(args, model, tokenizer, test_dataset, args.beta, group_label_map)
